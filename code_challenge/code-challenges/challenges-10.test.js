...
const count = (target, input) => {
  ...
  return input.reduce(
    (accumulator, current) =>
      accumulator + current.filter((ele) => ele === target).length,
    0
  );
};
...
let findMaleAndFemale = (data) => {
  ...
  return data
    .map((ele) =>
      ele.gender === "female" || ele.gender === "male" ? ele.name : false
    )
    .filter((ele) => ele)
    .join(" and ");
};
...
let findShortest = (data) => {
  ...
  return data.reduce((accumulator, current) =>
    Number(accumulator.height) < Number(current.height) ? accumulator : current
  ).name;
};
