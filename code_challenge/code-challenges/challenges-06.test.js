...
const updateNumbers = (obj) => {
  // Solution code here...
  const phone = [];
  Object.entries(obj).forEach(([key, value]) => phone.push(`${key}: ${value}`));
  return phone;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach((ele) => {
    if (Object.values(ele).some((ele) => ele == character)) hasChildren = true;
  });
  return hasChildren;
};
...
const hasChildrenEntries = (arr, character) => {
  // Solution code here...
  let hasChildren = false;
  Object.entries(arr).forEach(([key, ele]) => {
...
const totalCharacters = (arr) => {
  ...
  arr.forEach((ele) => {
    if (ele.name) total += 1;
    if (ele.spouse) total += 1;
    total += ele.children.length;
  });
  return total;
};
...
const houseSize = (arr) => {
  ...
  arr.forEach((ele) => {
    ...
    if (ele.spouse) total += 1;
    total += ele.children.length;
    sizes.push({ house: ele.house, members: total });
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach((ele) => {
    let total = 0;
    if (ele.name) total += 1;
    if (ele.spouse && !deceasedSpouses.includes(ele.spouse)) total += 1;
    total += ele.children.length;
    survivors.push({ house: ele.house, members: total });
  });
  return survivors;
};
